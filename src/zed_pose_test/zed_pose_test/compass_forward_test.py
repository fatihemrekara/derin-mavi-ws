...
class CompassForwardLogger(Node):
    def __init__(self):
        super().__init__('compass_forward_logger')
        ...
        self.log_file = open('/tmp/compass_forward_log.csv', 'w')
        self.log_file.write("Time(s),State,RelAlt(m),Compass_Hdg(deg)\n")
        
        self.timer = self.create_timer(0.1, self.control_loop)
        self.get_logger().info("Compass Forward Logger scripti baslatildi.")
        self.get_logger().info("Loglar /tmp/compass_forward_log.csv dosyasina kaydediliyor...")
    ...
    def control_loop(self):
        ...
        if self.state == 'STARTING':
            ...
            # rel_alt -1 metreyi gectiyse veya 15s gectiyse ileri gitmeye basla
            ...
            # ALT_HOLD modunda derinligi koruyup 5 saniye boyunca ileri gidecek
            rc = OverrideRCIn()
            rc.channels = [65535] * 18
            ...
            # Batıp 5 saniye ilerledikten sonra araci yuzeye cikarmak veya motoru durdurmak icin Disarm edelim
            if elapsed < 0.2:
                if self.arm_client.wait_for_service(timeout_sec=0.5):
                    req = CommandBool.Request()
            ...
            # Arac su yuzeyine cikarken pusula loglarini kaydetmeye devam et
            pass

def main(args=None):
    rclpy.init(args=args)
    node = CompassForwardLogger()
    try:
        rclpy.spin(node)
