...
class StraightBlindTestNode(Node):
    def __init__(self):
        ...
        self.zed_center_depth = 0.0  # (Virtual Sonar) ZED kamerasinin ortasina denk gelen derinlik
    ...
    def on_zed_depth(self, msg: Image):
        # 32FC1 format float32 array
        # Lazer yerine 20x20 piksellik saglam bir alan taramasi:
        if msg.encoding == '32FC1':
            center_row = msg.height // 2
            center_col = msg.width // 2
            
            window_size = 10  # Merkezden saga/sola 10px -> Toplam 20x20 kutu
            valid_depths = []
            ...
            for r in range(max(0, center_row - window_size), min(msg.height, center_row + window_size)):
                for c in range(max(0, center_col - window_size), min(msg.width, center_col + window_size)):
                    ...
                    if len(data_bytes) == 4:
                        ...
                        # Sizin kodunuzdaki gibi kararli filtre: NaN, Inf ve 10 cm'den kucuk gurultuleri at
    ...
    def control_loop(self):
        ...
        if self.state == 'DIVING':
            ...
            yaw_err_deg = math.degrees(normalize_angle(self.ref_heading_rad - self.heading_rad)) if self.ref_heading_rad else 0.0
