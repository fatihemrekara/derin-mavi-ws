...
class ShallowSquareBlindTestNode(Node):
    def __init__(self):
        super().__init__('shallow_square_blind_test')
        
        self.declare_parameter('rc_override_topic', '/mavros/rc/override')
        self.declare_parameter('control_rate_hz', 20.0)
        self.declare_parameter('fwd_pwm', 1680)
        self.declare_parameter('fwd_duration_ref', 25.0)
        self.declare_parameter('k_heading_fwd', 2.0)
        self.declare_parameter('target_depth', -0.35)
        ...
        self.target_depth = float(gp('target_depth'))
        ...
        self.log_filename = os.path.join(os.path.expanduser('~'), f'shallow_square_blind_log_{stamp}.csv')
        ...
        self.get_logger().info(f"Yapılandırılabilir Sığ Kare Sürüş (Shallow Square Blind) Testi Başladı. Hedef Derinlik: {self.target_depth} m")
    ...
    def control_loop(self):
        ...
        if self.state == 'DIVING':
            ...
            self.get_logger().info(f'Dalış: 1400 PWM, target_depth: {self.target_depth}, rel_alt: {self.rel_alt:.2f}m', throttle_duration_sec=2.0)
...
def main(args=None):
    ...
    node = ShallowSquareBlindTestNode()
